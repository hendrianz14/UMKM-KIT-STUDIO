  ...
  const handleSubmit = useCallback(
    async (event: React.FormEvent<HTMLFormElement>) => {
      ...
      try {
        ...
        if (infoTarget) infoTarget.textContent = '';
        ...
        if (!response.ok || !ok) {
          const message: string =
            (payload?.error && String(payload.error)) || 'Terjadi kesalahan saat masuk. Coba lagi.';
          if (errorTarget) {
        ...
        const redirectTarget: string = payload?.redirect || '/dashboard';
        router.push(redirectTarget);
      } catch (error) {
        console.error('Login error:', error);
        const fallbackMessage = 'Tidak dapat terhubung ke server. Silakan coba lagi.';
        if (errorTarget) {
          errorTarget.textContent = fallbackMessage;
        }
      } finally {
        setIsSubmitting(false);
      }
    },
    [formData.email, formData.remember, isSubmitting, router],
  );
