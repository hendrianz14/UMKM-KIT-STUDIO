import { Product, PriceType, StockStatus } from '@/types';

// NOTE: Pastikan path impor '@/types' sesuai dengan konfigurasi path alias di tsconfig.json Anda.
// Jika tidak menggunakan alias, ubah menjadi path relatif seperti '../types'.
...
export const getProductPriceRange = (product: Product) => {
    if (product.priceType === PriceType.SINGLE) {
        return { 
            ...
            displayStrikethroughPrice: product.strikethroughPrice ? formatCurrency(product.strikethroughPrice) : undefined,
    ...
    if (availableCombinations.length === 0) {
        const hasAskOnWA = product.variants.combinations.some(c => c.stockStatus === StockStatus.AVAILABLE && c.askOnWA);
        return { displayPrice: hasAskOnWA ? "Tanya di WA" : "Stok habis", isAskOnWA: hasAskOnWA };
    }

    const prices = availableCombinations.map(c => c.price);
    const minPrice = Math.min(...prices);
    const maxPrice = Math.max(...prices);

    if (minPrice === maxPrice) {
        const combo = availableCombinations[0];
        return { 
            displayPrice: formatCurrency(minPrice), 
            displayStrikethroughPrice: combo.strikethroughPrice ? formatCurrency(combo.strikethroughPrice) : undefined,
            isAskOnWA: false
        };
    }
...
export const getSortPrice = (product: Product): number => {
    ...
    const availablePrices = product.variants.combinations
        .filter(c => c.stockStatus === StockStatus.AVAILABLE && !c.askOnWA && c.price > 0)
        .map(c => c.price);
...
const profanityList = [
  ...
  'jembut', 'ngentot', 'asu', 'jancok', 'goblok', 'tolol', 'setan', 'iblis',
  'cuki', 'perek', 'lonte', 'tempik', 'ngewe',
  // English
  'fuck', 'shit', 'bitch', 'asshole', 'cunt', 'dick', 'pussy', 'bastard',
  'damn', 'hell', 'sex', 'sexy',
  // Universal sensitive topics
  'nazi', 'hitler', 'isis', 'teroris'
];
