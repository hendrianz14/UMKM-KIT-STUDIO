...
import { Product, ProductStatus } from '@/types/storefront.types';
import { getProductPriceRange } from '@/lib/utils.client';

interface ProductCardProps {
  product: Product;
  // Fix: Made onSelect optional to accommodate usage where navigation is handled by a parent component (e.g., Next.js Link).
  onSelect?: () => void;
}
...
const ProductCard: React.FC<ProductCardProps> = ({ product, onSelect }) => {
  ...
  return (
    <div 
      className={`bg-white rounded-lg shadow-md overflow-hidden transform transition-all duration-300 flex flex-col group ${isUnavailable || !onSelect ? 'cursor-default' : 'hover:-translate-y-1 cursor-pointer'}`}
      onClick={isUnavailable || !onSelect ? undefined : onSelect}
      role={isUnavailable || !onSelect ? undefined : "button"}
      tabIndex={isUnavailable || !onSelect ? -1 : 0}
      onKeyDown={(e) => { if (!isUnavailable && onSelect && (e.key === 'Enter' || e.key === ' ')) onSelect(); }}
    >
      ...
      <div className="p-4 flex-grow flex flex-col">
        ...
        <p className="mt-2 text-lg font-extrabold text-gray-900">{displayPrice}</p>
      </div>
    </div>
  );
};
