...
import { DashboardStatsData, Project, CreditHistoryItem, User } from '../lib/types';
...
const Footer = () => (
  <footer className="flex flex-col md:flex-row justify-between items-center text-center md:text-left gap-6 text-sm text-gray-500 pt-8 border-t border-gray-200">
    <p>Copyright 2025 UMKM KitStudio. Seluruh hak cipta dilindungi.</p>
...
export default function DashboardClient() {
  const { user, dashboardStats, projects, creditHistory } = useAppContext();
  ...
  const isReady = useMemo(() => Boolean(user && dashboardStats), [user, dashboardStats]);

  if (!isReady || !user || !dashboardStats) {
    return (
  ...
  return (
    <>
      <div className="max-w-7xl mx-auto animate-fadeInUp">
        ...
        <ProjectsSection projects={projects} onShareClick={handleShareClick} />
        <CreditsHistory history={creditHistory} />
        <Footer />
      </div>
