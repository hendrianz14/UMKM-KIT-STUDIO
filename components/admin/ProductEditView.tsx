...
async function compressImageFile(file: File, maxDimension: number = MAX_IMAGE_DIMENSION, quality: number = JPEG_QUALITY): Promise<string> {
...
const parseOptionalNumberFromInput = (value: string): number | undefined => {
  const sanitized = sanitizeNumberInput(value);
...
useEffect(() => {
    ...
    if (trimmedName.length < MIN_PRODUCT_NAME_LENGTH) {
        setNameError(`Nama produk minimal ${MIN_PRODUCT_NAME_LENGTH} karakter.`);
        return;
    }
...
const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    try {
      ...
      if (!storefrontSettings?.id) {
        alert("Storefront belum siap. Muat ulang halaman dan coba kembali.");
        return;
      }
      ...
      const response = await fetch("/api/storefront/upload-image", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          dataUrl: compressedDataUrl,
          storeId: storefrontSettings.id,
          fileName: file.name,
        }),
      });
      ...
      const message =
        error instanceof Error ? error.message : "Upload gambar gagal. Coba lagi.";
      alert(message);
    } finally {
  ...
  const addImage = ({ url, storagePath }: { url: string; storagePath?: string }) => {
    if (product.images.length >= 6) return;
    const newImage: ProductImage = { id: `img-${Date.now()}`, url, altText: '', storagePath };
  ...
  const removeImage = async (id: string) => {
    const targetImage = product.images.find(img => img.id === id);

    if (targetImage?.storagePath) {
      try {
        await fetch('/api/storefront/delete-image', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ storagePath: targetImage.storagePath }),
        });
      } catch (error) {
                        ...
                        <>
                            ...
                            />
                              <button
                                  ...
                                  className="border-2 border-dashed rounded-lg flex flex-col items-center justify-center h-full text-gray-500 hover:bg-light py-10 disabled:opacity-50 disabled:cursor-not-allowed">
                                  <ImagePlusIcon />
                     ...
                     <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                         <div>
                            ...
                            <input
                                ...
                                onChange={e => {
                                    ...
                                    setProduct(p => ({ ...p, price: numericValue }));
                                }}
                                className={inputStyles}
                            />
                         </div>
                        <div>
                            ...
                            <input
                                ...
                                inputMode="numeric"
                                pattern="[0-9]*"
                                value={product.strikethroughPrice ? String(product.strikethroughPrice) : ''}
                                onChange={e => {
                                    const numericValue = parseOptionalNumberFromInput(e.target.value);
                                    setProduct(p => ({ ...p, strikethroughPrice: numericValue }));
                                }}
                                className={inputStyles}
                            />
                        </div>
                        ...
                        {product.variants.combinations.length > 0 && (
                            <div className="overflow-x-auto">
                                <table className="min-w-full text-sm">
                                    ...
                                    value={combo.price ? String(combo.price) : ''}
                                    onChange={e => {
                                        const numericValue = parseNumberFromInput(e.target.value);
                                        updateCombination(combo.id, 'price', numericValue);
                                    }}
                                    className={`${inputStyles} w-24 disabled:bg-gray-100`}
                                />
                                                ...
                                                <td className="p-2 border-b">
                                                    <input
                                                        ...
                                                        inputMode="numeric"
                                                        pattern="[0-9]*"
                                                        value={combo.strikethroughPrice ? String(combo.strikethroughPrice) : ''}
                                                        onChange={e => {
                                                            const numericValue = parseOptionalNumberFromInput(e.target.value);
                                                            updateCombination(combo.id, 'strikethroughPrice', numericValue);
                                                        }}
